...
import { BodyTypes, IBodyTypes } from "types/profileTypes";

interface IProps {
  data: IBodyTypes[];
  isModalOpen: boolean;
  setBodyTypeModalOpen: (isModalOpen: boolean) => void;
  bodyType: string;
...
const ProfileBodyTypeModal: React.FC<IProps> = ({
  data,
  isModalOpen,
  setBodyTypeModalOpen,
  bodyType,
  setBodyType,
}) => (
  <>
        ...
        {data?.map((item, index) => (
          <Row
            ...
            onClick={() => {
              ...
              setBodyType(item.key);
            }}
          >
            <Text isSelected={item.key === bodyType}>
              {BodyTypes[item.key]}
            </Text>
          </Row>
        ))}
...
export default ProfileBodyTypeModal;
